...
(function () {
  ...
  function initPreschoolersSwiper() {
    ...
    //
    // Line ~3614: transform-origin: 50% XXXpx;
    //   → Use CONFIG.arcRadius.desktop (e.g., 900px)
    // Line ~3613: margin-top: -YYYpx;
    //   → Formula: -(arcRadius / 3.5) approx
    //   → Example: 900px → -260px
    //
    ...
    function snapToNearest(dragOffset) {
      ...
      updateCardPositions(snapTarget, CONFIG.snapDuration);
    ...
    function applyMomentum() {
      if (Math.abs(velocity) < 0.5) {
        ...
        const constrainedOffset = Math.max(minOffset, Math.min(maxOffset, currentRotation));
        snapToNearest(constrainedOffset);
        return;
      }
